<template>
  <div class="game-stream-page">
    ...
    <div
      v-if="!streamList.length && loadingSuccess"
      class="notice"
    >
      <h3>目前沒有Live頻道</h3>
    </div>
...
export default {
  ...
  methods: {
    getGameStreamList(mounted) {
      if (this.cursor === undefined) return;
      if (mounted) this.$store.commit('LOADING');
      this.streamList.push({}, {}, {}, {}, {});
      let api = `https://api.twitch.tv/helix/streams?game_id=${this.gameId}&first=25&language=${this.lang}&after=${this.cursor}`;
      if (this.lang === 'all') {
        api = `https://api.twitch.tv/helix/streams?game_id=${this.gameId}&first=25&after=${this.cursor}`;
      }
      this.$http.get(api, {
        headers: {
